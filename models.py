...
def load_pretrained(model_path, args, new_dataset_params=False):
    ...
    try:
        ...
        logger.warning(f"Error loading state dict for model {old_args.model}: {e}")
        ...
        logger.warning(f"file-model: {file_minus_model}")
    ...
    if new_dataset_params:
        ...
        if args.has_text_embeddings is not None and not args.has_text_embeddings and hasattr(model, "remove_text_head"):
            logger.info("Removing text head from model")
            model.remove_text_head()
        elif args.has_text_embeddings is not None and not args.has_text_embeddings:
            logger.error("Could not remove text head from model: no remove_text_head function implemented")
