...
class SwinTransformer(nn.Module, ResizingInterface):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.text_head = nn.Linear(self.num_features, text_embed_dim) if text_embed_dim > 0 else nn.Identity()
    ...
    def remove_text_head(self):
        if self.text_dim > 0:
            for name, _ in list(self.text_head.named_parameters()):
                del self.text_head._parameters[name]
    ...
    def forward(self, x, return_all_vals=False):
        ...
        x = torch.flatten(x, 1)
        if return_all_vals:
            return x, self.head(x), self.text_head(x)
        return self.head(x)
